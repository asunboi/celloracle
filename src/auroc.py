...
def plot_scatter_with_labels(result, output_file, label):
    ...
    for i in range(len(result)):
        ...
        if abs(x_val) > 1 and abs(x_val) < 2:
            plt.text(
                x_val, 
                y_val, 
                result['Gene'][i],
...
def plot_scatter_with_labels_y(result, output_file, label, y_value):
    """
    Plots a scatter plot with gene labels for points where the absolute x-value is greater than 1.
    ...
    for i in range(len(result)):
        x_val = result['Temp_Log2_Fold_Change'][i]
        y_val = result['logFC'][i]
        if abs(x_val) > 1 or abs(y_val) > y_value:
            if abs(x_val) < 2:
                plt.text(
                    x_val, 
                    y_val, 
...
goi_list= ["Nr2f1"]  # Genes of interest
    ...
    for sample in sample_list:
        for i in range(0, len(region_list)):
            ...
            for j in range(0, len(name_list)):
                ...
                deg_file_path = f"/gpfs/home/asun/jinlab/tsv/edgeR_LRT_with_sva.{goi}_{sample}.{region}.tsv"
                if os.path.exists(deg_file_path):
                    df = pd.read_csv(deg_file_path, sep='\t')
                    # Get Sean's DEG list
                ...
                if j == 0:
                    ...
                    plot_scatter_with_labels(result, f"/gpfs/home/asun/jinlab/graphs/rescale/{goi}_{sample}_{region}_{name}_rescale_scatter.pdf", "PerturbSeq")
                    plot_scatter_with_labels_y(result, f"/gpfs/home/asun/jinlab/graphs/rescale/{goi}_{sample}_{region}_{name}_rescale_1_scatter.pdf", "PerturbSeq", 1)
                    plot_scatter_with_labels_y(result, f"/gpfs/home/asun/jinlab/graphs/rescale/{goi}_{sample}_{region}_{name}_rescale_3_scatter.pdf", "PerturbSeq", 1)
                elif j == 1:
                    ctrl_fpr = temp_fpr
                    ctrl_tpr = temp_tpr
                    ctrl_roc_auc = temp_roc_auc
                    plot_scatter_with_labels(result, f"/gpfs/home/asun/jinlab/graphs/rescale/{goi}_{sample}_{region}_{name}_rescale_scatter.pdf", "Ctrl")
                    plot_scatter_with_labels_y(result, f"/gpfs/home/asun/jinlab/graphs/rescale/{goi}_{sample}_{region}_{name}_rescale_1_scatter.pdf", "PerturbSeq", 1)
                    plot_scatter_with_labels_y(result, f"/gpfs/home/asun/jinlab/graphs/rescale/{goi}_{sample}_{region}_{name}_rescale_3_scatter.pdf", "PerturbSeq", 1)
                else:
                    goi_fpr = temp_fpr
                    ...
                    plot_scatter_with_labels(result, f"/gpfs/home/asun/jinlab/graphs/rescale/{goi}_{sample}_{region}_{name}_rescale_scatter.pdf", goi)
                    plot_scatter_with_labels_y(result, f"/gpfs/home/asun/jinlab/graphs/rescale/{goi}_{sample}_{region}_{name}_rescale_1_scatter.pdf", "PerturbSeq", 1)
                    plot_scatter_with_labels_y(result, f"/gpfs/home/asun/jinlab/graphs/rescale/{goi}_{sample}_{region}_{name}_rescale_3_scatter.pdf", "PerturbSeq", 1)
